...
SS_ENDPOINT = "https://sky-scanner3.p.rapidapi.com/flights/search-roundtrip"  # Your TEQUILA_ENDPOINT
...
class FlightSearchSS:
        
       
    def check_flights(self, origin_city_code, destination_city_code, from_time, ret_from_time):
        """ This function checks for the available flights through a get() request """
        headers = {
        "X-RapidAPI-Key": "a9e5afa2a8msh24767c109148d00p1f31d7jsn9ace40f93553",
        "X-RapidAPI-Host": "sky-scanner3.p.rapidapi.com"
        }
        query = {
            "fromEntityId": origin_city_code,
            "toEntityId": destination_city_code,
            "departDate": from_time,
            "returnDate": ret_from_time,
            "stops": 'direct',
            "currency": "EUR"
        }

        response = requests.get(
            url= SS_ENDPOINT,  # search es el endpoint de SEARCH API
            headers=headers,
            params=query,
        )
        ...
        try:
            ...
            for itineraries in data:
                precio = itineraries["price"]["raw"]
                url = "https://www.ryanair.com/es/es"
                portal = "SkyScanner"
                aeropuerto_ida = itineraries["legs"][0]["origin"]["id"]
                
                fecha_ida = itineraries["legs"][0]["departure"].split("T")[0]
                fecha_ida = transformar_fecha(fecha_ida)
                ...
                fecha_ida_llegada = transformar_fecha(fecha_ida_llegada)
                ...
                fecha_vuelta = transformar_fecha(fecha_vuelta)
                ...
                fecha_vuelta_llegada = transformar_fecha(fecha_vuelta_llegada)
                ...
                vuelo = Vuelo(precio, url, portal, aeropuerto_ida, fecha_ida, hora_ida, fecha_ida_llegada, hora_ida_llegada, aeropuerto_vuelta, fecha_vuelta, hora_vuelta, fecha_vuelta_llegada, hora_vuelta_llegada)
                vuelos.append(vuelo)
                print(vuelo)    
        except IndexError:
            pass
...
if __name__ == "__main__":
    flight_search = FlightSearchSS()
    origin_city_code = "MAD"
    destination_city_code = "LGW"
    from_time = "2024-08-23"
    ret_from_time = "2024-08-31"

    vuelos = flight_search.check_flights(origin_city_code, destination_city_code, from_time, ret_from_time)
    if vuelos:
        for vuelo in vuelos:
            print(vuelo)
    else:
